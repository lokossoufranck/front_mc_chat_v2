  ...
  <div class="grid">
    <div class="col-6">
      <div class="card">
        ...
        <TabView scrollable>
          <TabPanel header="Tout">
            <div class="flex align-items-center justify-content-between mb-4">
              <i class="pi pi-comment p-text-secondary" style="font-size: 2rem" v-badge.secondary="latestInputmessages.length"></i>
              
         
						<span class="p-input-icon-right">
							<InputText type="text" placeholder="Rechercher" />
            ...
            <ul class="p-0 mx-0 mt-0 mb-4 list-none">
              ...
              >
                ...
                <span class="text-500 line-height-3">
                  ...
                  <br />
                  <span class="text-00">{{ item.body.substring(0, 40) }} ...</span>
                  <br />
          ...
          <TabPanel header="Nouv">
            <div class="flex align-items-center justify-content-between mb-4">
               <!-- <h5> {{latestInputmessages.filter(elt=>elt.etat==1).length }} Discussions</h5> -->
              <i class="pi pi-comment p-text-secondary" style="font-size: 2rem" v-badge.warning="latestInputmessages.filter(elt=>elt.etat==1).length"></i>
              <div>
                <Button
                  icon="pi pi-ellipsis-v"
                  class="p-button-text p-button-plain p-button-rounded"
            ...
            <ul class="p-0 mx-0 mt-0 mb-4 list-none">
              <li
                v-for="(item, index) in latestInputmessages.filter(elt=>elt.etat==1)"
                :key="index"
                class="
                  flex
                  align-items-center
          ...
          <TabPanel header="Ouvert">
            <div class="flex align-items-center justify-content-between mb-4">
              <i class="pi pi-comment p-text-secondary" style="font-size: 2rem" v-badge.info="latestInputmessages.filter(elt=>elt.etat==2).length"></i>
              <div>
                <Button
                  icon="pi pi-ellipsis-v"
                  class="p-button-text p-button-plain p-button-rounded"
            ...
            <ul class="p-0 mx-0 mt-0 mb-4 list-none">
              <li
                v-for="(item, index) in latestInputmessages.filter(elt=>elt.etat==2)"
                :key="index"
                class="
                  flex
                  align-items-center
          ...
          <TabPanel header="Non lue(s)">
            <div class="flex align-items-center justify-content-between mb-4">
              <i class="pi pi-comment p-text-secondary" style="font-size: 2rem" v-badge.danger="latestInputmessages.filter(elt=>elt.etat==3).length"></i>
              <div>
                <Button
                  icon="pi pi-ellipsis-v"
                  class="p-button-text p-button-plain p-button-rounded"
            ...
            <ul class="p-0 mx-0 mt-0 mb-4 list-none">
              <li
                v-for="(item, index) in latestInputmessages.filter(elt=>elt.etat==3)"
                :key="index"
                class="
                  flex
                  align-items-center
          ...
          <TabPanel header="Terminé">
            <div class="flex align-items-center justify-content-between mb-4">
               <i class="pi pi-comment p-text-secondary" style="font-size: 2rem" v-badge.success="latestInputmessages.filter(elt=>elt.etat==4).length"></i>
              <div>
                <Button
                  icon="pi pi-ellipsis-v"
                  class="p-button-text p-button-plain p-button-rounded"
            ...
            <ul class="p-0 mx-0 mt-0 mb-4 list-none">
              <li
                v-for="(item, index) in latestInputmessages.filter(elt=>elt.etat==4)"
                :key="index"
                class="
                  flex
                  align-items-center
        ...
        <ul class="p-0 mx-0 mt-0 mb-4 list-none">
          ...
          >
            ...
            <span class="text-500 line-height-3">
              <span class="text-blue-500">{{ item.wp_number_sender }}</span>
              <br />
              <span class="text-00">{{ item.body.substring(0, 15) }} </span>
              <br />
            ...
            <div class="card mb-0 bg-blue-500">
                  ...
                  <div>
                    ...
                    >
                      <Button
                        type="button"
                        label="Audio"
                        @click.prevent="playSound(API_URL + oneMsg.filename)"
                        icon="pi pi-play"
              ...
              <span class="text-green text-xs font-italic">
                {{ this.myDateAgoFormatter(oneMsg.created_at) }}
                 {{ this.myTimeFormatter(oneMsg.created_at) }} 
              </span>
            </div>
          </div>
        </div>
        ...
        >
          <div class="col-8 lg:col-8 xl:col-8">
            <div class="card mb-0">
              <div class="flex justify-content-between mb-3">
                <div>
                  <span class="block font-medium mb-3"> {{oneMsg.name}} </span>
...
export default {
  ...
  computed: {
    ...
    ...mapState({
     ...
     /* newChat: (state) => state.message.newChat,
      answeredChat: (state) => state.message.answeredChat,
      notAnsweredChat: (state) => state.message.notAnsweredChat,
      discussionsNouvelles: (state) =>
        state.message.latestInputmessages.filter((item) => {
          return item.etat == 1;
        }),
      discussionsOuvertes: (state) =>
        state.message.latestInputmessages.filter((item) => {
          return item.etat == 2;
        }),
      discussionsNonLues: (state) =>
        state.message.latestInputmessages.filter((item) => {
          return item.etat == 3;
        }),
      discussionsTerminees: (state) =>
        state.message.latestInputmessages.filter((item) => {
          return item.etat == 4;
        }),*/
  ...
  methods: {
    ...
    saveStatus() {
      ...
      if (!this.displayMessage.discussion_id) {
        ...
        const ostatus = new Object();
        const dis = new Object();
        
        ostatus.user_id = user.userId;
        ostatus.discussion_id = this.displayMessage.discussion_id;
      
        
        dis.id = this.displayMessage.discussion_id;
        dis.is_open=0;
        
        DiscussionService.updateDiscussion(dis).then((retour) => {
        //alert(1);
        console.log(retour);
          // if (retour.error == false) {
            this.$store.commit("message/setMode", "simple");
      ...
      if (this.body_new_msg.length > 0)
        MessageService.sendMessage(lemessage).then((response) => {
          ...
          setTimeout(() => {
            ...
            const o= new Object();
            o.user_id = user.userId;
            o.discussion_id = this.displayMessage.discussion_id;
            o.status_id = this.status_selected.id;
            this["message/getLatestInputmessages"](o);
          }, 300);
        });
    },
    ...
    myDateAgoFormatter(date) {
      const dateTimeAgo = moment(date).add(1, 'hours').locale("fr").fromNow();
      return dateTimeAgo;
    },
  
    myTimeFormatter(date) {
      const dateTimeAgo = moment(date).add(1, 'hours').locale("fr").format('hh:mm:ss');
      return dateTimeAgo;
    },
    isDarkTheme() {
